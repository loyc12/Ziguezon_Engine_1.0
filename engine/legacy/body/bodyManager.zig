...
pub const BodyManager = struct
{
  ...
  fn getNewId( self : *BodyManager ) u32
  {
    if( !self.isInit )
    {
      utl.qlog( .ERROR, 0, @src(), "Body manager is not initialized : returning id 0" );
      return 0;
    }
    self.maxId += 1;
    return self.maxId;
  ...
  pub fn recalcMaxId( self : *BodyManager ) void
  {
    if( !self.isInit )
    {
      utl.qlog( .ERROR, 0, @src(), "Body manager is not initialized : cannot recalculate maxId" );
      return;
    }
    var newMaxId: u32 = 0;
  ...
  pub fn isIdValid( self : *BodyManager, id : u32 ) bool
  {
    if( id <= 0 )
    {
      utl.qlog( .WARN, 0, @src(), "Body Id cannot be 0 or less" );
      return false;
    }
    if( id > self.maxId )
    {
      utl.log( .WARN, 0, @src(), "Body Id {d} is greater than maxId {d}", .{ id, self.maxId });
      return false;
    }
    return true;
  }
  ...
  fn getIndexOf( self : *BodyManager, id : u32 ) ?usize
  {
    if( !self.isInit )
    {
      utl.qlog( .ERROR, 0, @src(), "Body manager is not initialized : returning null" );
      return null;
    }

    if( !self.isIdValid( id ))
    {
      utl.log( .WARN, 0, @src(), "Body Id {d} is not valid", .{ id });
      return null;
    }
    ...
    utl.log( .TRACE, 0, @src(), "Body with Id {d} not found", .{ id });
    return null;
  }
  ...
  fn isIndexValid( self : *BodyManager, index : ?usize ) bool
  {
    ...
    {
      utl.qlog( .WARN, 0, @src(), "No bodyList available" );
      return false;
    }
    if( index == null )
    {
    ...
    {
      utl.log( .WARN, 0, @src(), "Index {d} is out of bounds ( 0 to {d} )", .{ index, self.bodyList.len });
      return false;
    }
    return true;
  }
  ...
  {
    utl.qlog( .TRACE, 0, @src(), "# Initializing Body manager..." );

    if( self.isInit )
    {
      utl.qlog( .WARN, 0, @src(), "@ Body manager is already initialized" );
      return;
    }
    ...
    utl.qlog( .INFO, 0, @src(), "$ Body manager initialized !\n" );
  }

  pub fn deinit( self : *BodyManager ) void
  {
    utl.qlog( .TRACE, 0, @src(), "# Deinitializing Body manager..." );

    if( !self.isInit )
    {
      utl.qlog( .WARN, 0, @src(), "@ Body manager was not initialized" );
      return;
    }
    ...
    utl.qlog( .INFO, 0, @src(), "$ Body manager deinitialized !\n" );
  }
  ...
  {
    utl.qlog( .TRACE, 0, @src(), "Adding new Body" );

    if( !self.isInit )
    {
      utl.qlog( .WARN, 0, @src(), "Body manager is not initialized" );
      return null;
    }

    var tmp = Body.createBodyFromParams( params ) orelse
    {
      utl.qlog( .ERROR, 0, @src(), "Failed to create Body from params" );
      return null;
    };
    ...
    {
      utl.log( .ERROR, 0, @src(), "Failed to add Body: {}", .{ err });
      return null;
    };
  ...
  {
    utl.qlog( .TRACE, 0, @src(), "Creating default Body" );
  ...
  {
    utl.log( .TRACE, 0, @src(), "Getting Body with Id {d}", .{ id });

    const index = self.getIndexOf( id ) orelse
    {
      utl.log( .TRACE, 0, @src(), "Body with Id {d} not found : returning null", .{ id });
      return null;
    };
  ...
  pub fn delBody( self : *BodyManager, id : u32 ) void
  {
    ...
    if( index == null )
    {
      utl.log( .WARN, 0, @src(), "Body with Id {d} not found : returning", .{ id });
      return;
    }
    ...
    utl.log( .DEBUG, 0, @src(), "Body with Id {d} deleted", .{ id });
  }

  pub fn deleteAllMarkedBodies( self : *BodyManager ) void
  {
    utl.qlog( .TRACE, 0, @src(), "Deleting all Bodies marked for deletion" );

    if( !self.isInit )
    {
      utl.qlog( .WARN, 0, @src(), "Body manager is not initialized : returning" );
      return;
    }
  ...
  {
    utl.qlog( .TRACE, 0, @src(), "Rendering Body hitboxes" );
  ...
  pub fn renderActiveBodies( self : *BodyManager, ng : *eng.Engine ) void // TODO : have this take in a renderer construct and pass it to Body.renderGraphics()
  {
    utl.qlog( .TRACE, 0, @src(), "Rendering active Bodies" );
  ...
  pub fn tickActiveBodies( self : *BodyManager, ng : *eng.Engine ) void
  {
    utl.qlog( .TRACE, 0, @src(), "Ticking active Bodies" );
  ...
  pub fn collideActiveBodies( self : *BodyManager, ng : *eng.Engine ) void // TODO : make it actually collide body, instead of just logging overlap
  {
    utl.qlog( .TRACE, 0, @src(), "Coliding active Bodies" );
    ...
    if( eng.CNFGS.AutoApply_Body_Collision ){ for( self.bodyList.items, 0 .. )| *e1, index |{ if( e1.isActive() )
    {
      ...
      for( self.bodyList.items[ index + 1 .. ])| e2 |{ if( e2.isActive() )
      {
        ...
        {
          utl.log( .DEBUG, 0, @src(), "Collision detected between Body {d} and {d} with magnitude {d}:{d}", .{ e1.id, e2.id, overlap.x, overlap.y });
          utl.log( .DEBUG, 0, @src(), "Body {d} position: {d}:{d}, Body {d} position: {d}:{d}", .{ e1.id, e1.pos.x, e1.pos.y, e2.id, e2.pos.x, e2.pos.y });
          utl.log( .DEBUG, 0, @src(), "Body {d} scale:    {d}:{d}, Body {d} scale:    {d}:{d}", .{ e1.id, e1.scale.x, e1.scale.y, e2.id, e2.scale.x, e2.scale.y });
        }
      }}
    }}}
  }
