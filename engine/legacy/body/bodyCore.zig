...
//pub inline fn isAnimate( self : *const Body ) bool { return self.hasFlag( e_bdy_flags.ANIMATE ); }
  ...
  //utl.log( .INFO, 0, @src(), "Created body with shape : {s}", .{ @tagName( params.shape )});
  ...
  pub fn createBodyFromFile( filePath : []const u8 ) ?Body
  {
    ...
    utl.qlog( .ERROR, 0, @src(), "Body loading from file is not yet implemented" );
    return null;
  }
  ...
  pub fn moveSelf( self : *Body, sdt : f32 ) void
  {
    if( !self.isMobile() )
    {
      utl.log( .TRACE, self.id, @src(), "Body {d} is not mobile and cannot be moved", .{ self.id });
      return;
    }
    utl.log( .TRACE, self.id, @src(), "Moving Body {d} by velocity {d}:{d} with acceleration {d}:{d} over time {d}", .{ self.id, self.vel.x, self.vel.y, self.acc.x, self.acc.y, sdt });
  ...
  pub inline fn setLeftX(   self : *Body, leftX   : f64 ) void { self.hitbox.center.x = utl.getCenterXFromLeftX(   leftX,   self.hitbox.scale ); }
  pub inline fn setRightX(  self : *Body, rightX  : f64 ) void { self.hitbox.center.x = utl.getCenterXFromRightX(  rightX,  self.hitbox.scale ); }
  pub inline fn setTopY(    self : *Body, topY    : f64 ) void { self.hitbox.center.y = utl.getCenterYFromTopY(    topY,    self.hitbox.scale ); }
  pub inline fn setBottomY( self : *Body, bottomY : f64 ) void { self.hitbox.center.y = utl.getCenterYFromBottomY( bottomY, self.hitbox.scale ); }

  pub inline fn setTopLeft(     self : *Body, topLeftPos     : Vec2 ) void { self.hitbox.center = utl.getCenterFromTopLeft(     topLeftPos,     self.hitbox.scale ); }
  pub inline fn setTopRight(    self : *Body, topRightPos    : Vec2 ) void { self.hitbox.center = utl.getCenterFromTopRight(    topRightPos,    self.hitbox.scale ); }
  pub inline fn setBottomLeft(  self : *Body, bottomLeftPos  : Vec2 ) void { self.hitbox.center = utl.getCenterFromBottomLeft(  bottomLeftPos,  self.hitbox.scale ); }
  pub inline fn setBottomRight( self : *Body, bottomRightPos : Vec2 ) void { self.hitbox.center = utl.getCenterFromBottomRight( bottomRightPos, self.hitbox.scale ); }
