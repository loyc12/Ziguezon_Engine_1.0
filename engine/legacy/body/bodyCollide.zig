...
{
  utl.log( .TRACE, e1.id, @src(), "Checking if Body {d} overlaps with {d}", .{ e1.id, e2.id });
...
{
  utl.log( .TRACE, e1.id, @src(), "Getting overlap between Body {d} and {d}", .{ e1.id, e2.id });

  if( e1.id == e2.id )
  {
    utl.qlog( .WARN, e1.id, @src(), "Attempting to get overlap of the same body : returning null" );
    return null;
  }
...
{
  utl.log( .TRACE, e1.id, @src(), "Checking collision between Body {d} and {d}", .{ e1.id, e2.id });

  if( !e1.isSolid() or !e2.isSolid() )
  {
    utl.qlog( .DEBUG, e1.id, @src(), "One of the bodies is not solid : returning" );
    return false; // No collision if either Body is not solid
  }

  if( e1.id == e2.id ) // Check if the bodies are the same
  {
    utl.qlog( .DEBUG, e1.id, @src(), "Bodies are the same : returning" );
    return false;
  }

  if( !isOverlapping( e1, e2 ))
  {
    utl.qlog( .DEBUG, e1.id, @src(), "Bodies are not overlapping : returning" );
    return false;
  }
  ...
  if( overlap == null )
  {
    utl.qlog( .DEBUG, e1.id, @src(), "No overlap detected : returning" );
    return false;
  }
  ...
  utl.qlog( .TRACE, e1.id, @src(), "Collision detected and resolved : returning" );
  return true;
}
