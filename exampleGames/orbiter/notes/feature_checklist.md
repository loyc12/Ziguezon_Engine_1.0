# Orbiter Feature Checklist

Comprehensive snapshot of this game's features, derived from a sweep of the current `.zig` sources (not from prior intent). Whenever this drifts from code, trust the code and refresh this file.

For implementation strategy and ordering, see the [roadmaps](roadmaps/) folder. For the actionable short-term checklist, see [immediate_todo.md](immediate_todo.md). For player-facing design, see [design_document.md](design_document.md). For engineering conventions, see [design_philosophy.md](design_philosophy.md).
...
* `[ x ]` Component registry registration and deallocation for all stores
...
* `[ x ]` Pause toggle (`P`), target cycling (`J` / `K`), camera follow (`F`), speed change (`U` / `I`)
...
* `[ o ]` `StellarBodyName` enum with SOL + MERCURY + VENUS + TERRA + LUNA + MARS + PHOBOS + DEIMOS + 7 main-belt bodies
* `[ x ]` `StellarBodyType` enum (STAR, PLANET, PLANETOID, MOON, MOONLET, ASTEROID, COMET) with per-type display size / colour / LP count
* `[ x ]` Stellar data grid populated with real mass / radius / periapsis / apoapsis / longitude / type for all initial live entries
...
* `[   ]` Using real units for sunshine strenght
...
* `[ x ]` `STRESS_TEST` flag also spawns every other legal econ on every body at 100k pop (`stateInjects.zig`)

## Resource Layer (`data/resourceData.zig`)

* `[ x ]` `ResType` enum: WORK, FUEL, FOOD, WATER, POWER, ORE, INGOT, PART
* `[ x ]` `ResType.getInfStore()` mapping (WORK → HOUSING, all else → DEPOT)
...
* `[ ? ]` Multiple PopType variants (listed in roadmap suggestions)
* `[ ? ]` Pop subtype gating of industry activity

## Infrastructure Layer (`data/infrastructureData.zig`)

* `[ x ]` `InfType` enum: HABITAT, DEPOT, HOUSING, ASSEMBLY
...
* `[ . ]` BATTERY / TANKS infrastructure (commented; DEPOT currently covers all res with `STORE_RATE` proxy)
...
* `[ x ]` `updateInfUsage` computes USE_LVL for HOUSING (pop / cap), HABITAT (areaUsed / habitatArea or fallback), DEPOT (max resC / resL ratio)
...
* `[ x ]` Resource chain depth (ORE → INGOT → PART, FUEL via refinery)
