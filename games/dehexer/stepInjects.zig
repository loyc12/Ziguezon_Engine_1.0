...
pub fn OnFrameUpdate( ng : *eng.Engine ) void
{
  ...
  {
    if( utl.ray.isKeyPressed( utl.ray.KeyboardKey.enter ) or utl.ray.isKeyPressed( utl.ray.KeyboardKey.kp_enter ))
    {
      HAS_WON = false;
      IS_INIT = false;
  ...
  if( utl.ray.isMouseButtonPressed( utl.ray.MouseButton.left ) or utl.ray.isMouseButtonPressed( utl.ray.MouseButton.right ))
  {
    ...
    if( utl.ray.isMouseButtonPressed( utl.ray.MouseButton.left  )){ leftCLickTile(  ng, grid, clickedTile ); }
    if( utl.ray.isMouseButtonPressed( utl.ray.MouseButton.right )){ rightCLickTile( ng, grid, clickedTile ); }
  }
  ...
  if( !IS_INIT )
  {
    ...
    {
      DIFFICULTY = utl.clmp( DIFFICULTY + 2.0, 2.0, 32.0 );
    //DIFFICULTY = @min( @max( DIFFICULTY + 2.0, 10.0 ), 30.0 );
    }
    else if( utl.ray.isKeyPressed( utl.ray.KeyboardKey.down ))
    {
      DIFFICULTY = utl.clmp( DIFFICULTY - 2.0, 2.0, 32.0 );
    //DIFFICULTY = @min( @max( DIFFICULTY - 2.0, 10.0 ), 30.0 );
    }
    ...
    {
      LIFE_COUNT = utl.clmp( LIFE_COUNT - 1, 1, 12 );
    }
    else if( utl.ray.isKeyPressed( utl.ray.KeyboardKey.right ))
    {
      LIFE_COUNT = utl.clmp( LIFE_COUNT + 1, 1, 12 );
    }
  }
...
pub fn OnRenderOverlay( ng : *eng.Engine ) void
{
  ...
  utl.sDraw.textCenter( &mineBuff, .new( screenCenter.x * 0.75, screenCenter.y * 0.1 ), text_scale * 2.0, utl.Colour.red   );
  utl.sDraw.textCenter( &lifeBuff, .new( screenCenter.x * 1.25, screenCenter.y * 0.1 ), text_scale * 2.0, utl.Colour.green );
  ...
  for( 0 .. grid.getTileCount() )| index |
  {
    ...
    else if( tile.colour.isEq( .mBlue  )){ utl.sDraw.textCenter( "3", tileCenter, text_scale, .white ); }

    else if( tile.colour.isEq( .yellow )){ utl.sDraw.textCenter( "1", tileCenter, text_scale, .black ); }
    else if( tile.colour.isEq( .orange )){ utl.sDraw.textCenter( "2", tileCenter, text_scale, .black ); }
    else if( tile.colour.isEq( .red    )){ utl.sDraw.textCenter( "3", tileCenter, text_scale, .black ); }
    ...
    const numCol = switch( nMineCount )
    {
      ...
      6    => utl.Colour.orange,
      7    => utl.Colour.red,
      8    => utl.Colour.fuchsia,
      9    => utl.Colour.purple,
      else => utl.Colour.indigo,
    };

    utl.sDraw.textCenter( &numBuff, tileCenter, text_scale, numCol );
  }

  if( !IS_INIT )
  {
    utl.sDraw.coverScreenWithCol( utl.Colour.new( 0, 0, 0, 32 ));

    utl.sDraw.textCenter( diffName, .new( screenCenter.x,screenCenter.y * 0.5 ), text_scale * 3.0, .yellow );

    utl.sDraw.textCenter( "Use up & down arrows to change mine count ( difficulty )",                         .new( screenCenter.x, screenCenter.y * 0.80 ), text_scale * 2.0, .red    );
    utl.sDraw.textCenter( "Use left & right arrows to change life count",                                     .new( screenCenter.x, screenCenter.y * 1.00 ), text_scale * 2.0, .green  );
    utl.sDraw.textCenter( "Click any cell to start",                                                          .new( screenCenter.x, screenCenter.y * 1.50 ), text_scale * 2.0, .yellow );
    utl.sDraw.textCenter( "Dehexer plays like classic minesweeper, with two exception :",                     .new( screenCenter.x, screenCenter.y * 1.65 ), text_scale * 1.0, .nWhite );
    utl.sDraw.textCenter( "Mines can count for either 1, 2 or 3 'damage', which impacts the displayed value", .new( screenCenter.x, screenCenter.y * 1.75 ), text_scale * 1.0, .nWhite );
    utl.sDraw.textCenter( "You also only lose the game once you take more damage than you have lives",        .new( screenCenter.x, screenCenter.y * 1.85 ), text_scale * 1.0, .nWhite );
  }
  ...
  if( LIFE_COUNT <= 0 or HAS_WON )
  {
    ...
    utl.sDraw.coverScreenWithCol( utl.Colour.new( 0, 0, 0, 192 ));

    if( HAS_WON )
    {
      utl.sDraw.textCenter( "W + SKILLFUL + HELL YEAH + ROFL + STAY GLAD",    .new( screenCenter.x, screenCenter.y * 0.90 ), end_text_scale,       .green );
      utl.sDraw.textCenter( "Press Enter to Restart, champ",                  .new( screenCenter.x, screenCenter.y * 1.10 ), end_text_scale * 0.5, .yellow );
    }
    else
    {
      utl.sDraw.textCenter( "L + SKILL ISSUE + WOMP WOMP + LMFAO + STAY MAD", .new( screenCenter.x, screenCenter.y * 0.90 ), end_text_scale,       .red );
      utl.sDraw.textCenter( "Press Enter to Restart, loser",                  .new( screenCenter.x, screenCenter.y * 1.10 ), end_text_scale * 0.5, .yellow );
    }
  }
