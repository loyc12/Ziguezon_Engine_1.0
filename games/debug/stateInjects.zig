...
pub fn OnGameOpen( ng : *eng.Engine ) void
{
  ...
  )| bdy |{ EXAMPLE_BDY_ID = bdy.id; } else { utl.qlog( .ERROR, 0, @src(), "Failed to create example body" ); }
  ...
  )| bdy |{ EXAMPLE_RLIN_ID = bdy.id; } else { utl.qlog( .ERROR, 0, @src(), "Failed to create example radius line" ); }
  ...
  )| bdy |{ EXAMPLE_DLIN_ID = bdy.id; } else { utl.qlog( .ERROR, 0, @src(), "Failed to create example diametre line" ); }
  ...
  )| bdy |{ EXAMPLE_TRIA_ID = bdy.id; } else { utl.qlog( .ERROR, 0, @src(), "Failed to create example triangle" ); }
  ...
  )| bdy |{ EXAMPLE_RECT_ID = bdy.id; } else { utl.qlog( .ERROR, 0, @src(), "Failed to create example rectangle" ); }
  ...
  )| bdy |{ EXAMPLE_HEXA_ID = bdy.id; } else { utl.qlog( .ERROR, 0, @src(), "Failed to create example hexagon" ); }
  ...
  )| bdy |{ EXAMPLE_ELLI_ID = bdy.id; } else { utl.qlog( .ERROR, 0, @src(), "Failed to create example ellipse" ); }
