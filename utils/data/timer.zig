...
pub const TimeVal = struct
{
  ...
  pub inline fn timeSince( since : TimeVal ) TimeVal
  {
    if( !since.isSet() )
    {
      utl.qlog( .WARN, 0, @src(), "Tried to get time since on an uninitialized TimeVal" );
      return .{};
    }
  ...
  pub inline fn timeUntil( until : TimeVal ) TimeVal
  {
    if( !until.isSet() )
    {
      utl.qlog( .WARN, 0, @src(), "Tried to get time until on an uninitialized TimeVal" );
      return .{};
    }
  ...
  pub inline fn timeDiff( t1 : TimeVal, t2 : TimeVal ) TimeVal
  {
    ...
    {
      utl.qlog( .WARN, 0, @src(), "Tried to get time diff with an uninitialized TimeVal" );
      return .{};
    }
  ...
  pub inline fn new( value : anytype ) TimeVal
  {
    switch( @typeInfo( @TypeOf( value )))
    {
      ...
      {
        utl.log( .WARN, 0, @src(), "Tried to get TimeVal from unsuported type {}", @typeInfo( @TypeOf( value )));
        return .{};
      },
    }
  }
  ...
  pub fn setTo( self : *TimeVal, newValue : anytype ) void
  {
    switch( @typeInfo( @TypeOf( newValue )))
    {
      ...
      {
        utl.log( .WARN, 0, @src(), "Tried to set TimeVal via unsuported type {}", @typeInfo( @TypeOf( newValue )));
        return .{};
      },
    }
  }
  pub fn convTo( self : *const TimeVal, comptime retType : type ) retType
  {
    switch( @typeInfo( retType ))
    {
      ...
      {
        utl.log( .WARN, 0, @src(), "Tried to convert TimeVal to unsuported type {}", @typeInfo( retType ));
        return .{};
      },
    }
  }
...
pub const Timer = struct
{
  ...
  fn incrementBy( self : *Timer, delta : TimeVal ) void
  {
    ...
    {
      utl.log( .WARN, 0, @src(), "Tried to update Timer with a negative delta time ({d})", .{ delta.value });
    }
  ...
  fn tryLap( self : *Timer ) bool
  {
    ...
    if( !self.canLoop() )
    {
      ...
      {
        if( self.isDebug() ){ utl.log( .DEBUG, 0, @src(), "Timer completed its duration of {d}ns", .{ self.duration.value }); }
        self.stopSelf();
        return true;
      }
      return false;
  ...
  fn doLap( self : *Timer ) bool
  {
    ...
    {
      if( self.isDebug() ){ utl.log( .DEBUG, 0, @src(), "Timer completed a lap of {d}ns", .{ self.duration.value }); }
  ...
  pub inline fn getLapDuration( self : *const Timer ) TimeVal
  {
    if( !self.hasDuration() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get lap duration with no duration" ); }
      return .{};
    }
  ...
  pub inline fn getLapProgressFactor( self : *const Timer ) f32
  {
    if( !self.hasDuration() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get lap progress factor with no duration" ); }
      return 0.0;
    }
  ...
  pub inline fn getTotalProgress( self : *const Timer ) TimeVal
  {
    if( !self.hasTrueLoop() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get total progress on a non-looping Timer" ); }
  ...
  pub inline fn getTotalDuration( self : *const Timer ) TimeVal
  {
    if( !self.hasTrueLoop() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get total duration on a non-looping Timer" ); }
      return self.getLapDuration();
    }

    if( !self.hasDuration() or !self.hasLapLimit() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get total duration on a looping Timer with no duration and/or lap limit" ); }
      return .{};
    }
  ...
  pub inline fn getTotalProgressFactor( self : *const Timer ) f32
  {
    if( !self.hasTrueLoop() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get total progress factor on a non-looping Timer" ); }
      return self.getLapProgressFactor();
    }

    if( !self.hasDuration() or !self.hasLapLimit() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get total progress factor on a looping Timer with no duration and/or lap limit" ); }
      return 0.0;
    }
  ...
  pub inline fn getRemainingLapCount( self : *const Timer ) u32
  {
    if( !self.hasLapLimit() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get remaining laps with no lap limit" ); }
      return 0;
    }
  ...
  pub inline fn getRemainingLapTime( self : *const Timer ) TimeVal
  {
    if( !self.hasDuration() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get remaining lap time with no duration" ); }
      return .{};
    }
  ...
  pub inline fn getRemainingTotalTime( self : *const Timer ) TimeVal
  {
    if( !self.hasTrueLoop() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get remaining total time on a non-looping Timer" ); }
      return self.getRemainingLapTime();
    }

    if( !self.hasDuration() or !self.hasLapLimit() )
    {
      if( self.isDebug() ){ utl.qlog( .DEBUG, 0, @src(), "Tried to get remaining total time on a looping Timer with no duration and/or lap limit" ); }
      return .{};
    }
