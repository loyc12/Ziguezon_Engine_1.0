...
pub const VecA = struct
{
  ...
  pub inline fn fromAngleDeg( a : Angle ) VecA { return fromAngle( utl.DtR( a )); }
  ...
  pub inline fn fromAngleDegScaled( a : Angle, scale : VecA ) VecA { return fromAngleScaled( utl.DtR( a ), scale ); }
  ...
  pub inline fn isZero( self : VecA ) bool { return utl.isFltZr( self.x ) and utl.isFltZr( self.y ); }
  pub inline fn isIso(  self : VecA ) bool { return utl.isFltEq( self.x, self.y ); }

  pub inline fn isEq(   self : VecA, other : VecA ) bool { return utl.isFltEq( self.x, other.x ) and utl.isFltEq( self.y, other.y ) and self.a.isEq( other.a ); }
  ...
  pub inline fn div( self : VecA, other : VecA ) ?VecA
  {
    ...
    {
      utl.qlog( .ERROR, 0, @src(), "Division by zero in VecA.div()" );
      return null;
    }
  ...
  pub inline fn divVal( self : VecA, val : f64 ) ?VecA
  {
    if( utl.isFltZr( val ))
    {
      utl.qlog( .ERROR, 0, @src(), "Division by zero in VecA.divVal()" );
      return null;
    }
  ...
  pub inline fn lerp(  self : VecA, other : VecA, t : f64 ) VecA
  {
    ...
    const r : f64 = utl.lerp( self.a.r, other.a.r, t );
  ...
  pub fn normToLen( self : VecA, newLen : f64 ) VecA
  {
    ...
    {
      utl.qlog( .WARN, 0, @src(), "Normalizing a VecA to 0" );
      return .{};
    }
    ...
    {
      utl.qlog( .WARN, 0, @src(), "Normalizing a 0:0 VecA" );
      return .{};
    }
  ...
  pub inline fn rotDeg( self : VecA, d : f64   ) VecA { return self.rot( .{ .r = utl.DtR( d )}); }
