...
const GRAD_BITS  = 8;
const GRAD_COUNT = utl.pow2( GRAD_BITS );
...
pub const gradients : [ GRAD_COUNT ]Vec2 = blk:
{
  ...
  {
    const angle = utl.TAU * (( @as( f32, i ) + 0.5 ) / @as( f32, GRAD_COUNT ));
    table[ i ] =
    .{
      .x = @cos( angle ),
      .y = @sin( angle ),
...
pub const Noise2D = struct
{
  ...
  {
    return( utl.R2 * self.baseSample( pos )); // NOTE : Compensating for output range of [-~0.707, +~0.707]
  }
  ...
  inline fn baseSample( self : Noise2D, pos : Vec2 ) f32
  {
    ...
    const nx0 = utl.lerp( n00, n10, u );
    const nx1 = utl.lerp( n01, n11, u );

    return utl.lerp( nx0, nx1, v );
  }
