...
pub fn getRelTilePos( tlmp : *const Tilemap, gridCoords : Coords2 ) ?VecR
{
  ...
  switch( tlmp.tileShape )
  {
    ...
    .HEX1 => // Flat top hexagon
    {
      var rowOffset : f32 = 0.0;
      if( 1 == @mod( gridCoords.x, 2 )) rowOffset = 0.5; // keeps the overall grid square-ish

      pos = VecR.new( X * 1.5, ( Y + rowOffset ) * R3, 0 );
    },

    .HEX2 => // Pointy top hexagon
    {
      var rowOffset : f32 = 0.0;
      if( 1 == @mod( gridCoords.y, 2 )) rowOffset = 0.5; // keeps the overall grid square-ish

      pos = VecR.new(( X + rowOffset ) * R3, Y * 1.5, 0 );
    },
  }
