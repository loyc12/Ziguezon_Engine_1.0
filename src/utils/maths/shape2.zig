...
inline fn getRamanujanFactor( a : f64, b : f64 ) f64
{
  ...
  if( utl.isFltZr( sum )) return 1.0;
  if( utl.isFltZr( dif )) return 1.0; // Math would result in 1.0 anyways
...
pub const Shape2D = enum( u8 )
{
  ...
  pub inline fn getEdgeCount( self : Shape2D ) u16 // Aka "N"
  {
    return switch( self )
    {
      ...
      .DECA => 10,
      .DODE => 12,
      .ELLI => eng.G_CNFGS.Graphic_Ellipse_Facets, // 64 by default
  ...
  pub inline fn getPerim( self : Shape2D, s : Vec2 ) f64
  {
    ...
    switch( self )
    {
      ...
      .ELLI =>
      {
        ...
        if( utl.isFltEq( rX, rY )) // Circle circumference from radius
        {
          return regP;
        }
      ...
      //const regP = ( 2.0 * N ) * rM * @sin( PI / N );
        ...
        if( utl.isFltEq( rX, rY )) // Regular polygon perimeter from circumradius ( trigonometric form )
        {
          return regP;
        }
