...
{
  fillCol        : utl.Colour = utl.Colour.nBlack.setA( 230 ),
  fillHoverCol   : utl.Colour = utl.Colour.sGray.setA(  240 ),
  fillPressedCol : utl.Colour = utl.Colour.sGray.setA(  245 ),

  edgeCol        : utl.Colour = utl.Colour.lGray,
  edgeFocusCol   : utl.Colour = utl.Colour.pGold,

  textCol        : utl.Colour = utl.Colour.nWhite,
  mutedTextCol   : utl.Colour = utl.Colour.lGray,
  accentCol      : utl.Colour = utl.Colour.pTeal,

  lineWidth : f64  = 2.0,
  fontSize  : f64  = 16.0,

  pub inline fn fillForState( self : UiStyle, hovered : bool, pressed : bool ) utl.Colour
  {
  ...
  pub fn forKind( kind : UiNodeKind ) UiStyle
  {
    ...
    switch( kind )
    {
      .root, .panel =>
      {
        style.fillCol      = utl.Colour.nBlack.setA( 230 );
        style.edgeCol      = utl.Colour.mGray;
        style.mutedTextCol = utl.Colour.lGray;
      },

      .label =>
      {
        style.fillCol = utl.Colour.transpa;
        style.edgeCol = utl.Colour.transpa;
      },

      .button =>
      {
        style.fillCol        = utl.Colour.sGray.setA( 240 );
        style.fillHoverCol   = utl.Colour.sGray.setA( 245 );
        style.fillPressedCol = utl.Colour.dGray.setA( 250 );
        style.edgeCol        = utl.Colour.lGray;
      },

      .checkbox =>
      {
        style.fillCol        = utl.Colour.nBlack.setA( 230 );
        style.fillHoverCol   = utl.Colour.sGray.setA( 240 );
        style.fillPressedCol = utl.Colour.sGray.setA( 245 );
      },

      .popup =>
      {
        style.fillCol      = utl.Colour.nBlack.setA( 248 );
        style.edgeCol      = utl.Colour.pGray;
        style.edgeFocusCol = utl.Colour.pGold;
      },

      .window =>
      {
        style.fillCol      = utl.Colour.nBlack.setA( 245 );
        style.edgeCol      = utl.Colour.pOrange;
        style.edgeFocusCol = utl.Colour.pGold;
      },
      ...
      .scrollArea =>
      {
        ...
        style.edgeCol      = utl.Colour.sGray;
        style.edgeFocusCol = utl.Colour.pGold;
      },
...
pub const UiNodeOpts = struct
{
  ...
  desiredSize : utl.Vec2 = .new( 160.0, 32.0 ),
...
pub inline fn boxFromTopLeft( topLeft : utl.Vec2, size : utl.Vec2 ) utl.Box2
{
...
pub inline fn boxSize( box : utl.Box2 ) utl.Vec2
{
  return box.getSize();
}
