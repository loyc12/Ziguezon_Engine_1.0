...
pub const BuildEntry = struct
{
  ...
  pub inline fn getBuildableWithRes( self : *const BuildEntry, resT : ResType ) f64
  {
    ...
    if( resCost < utl.EPS ){ return self.unitCount; }
  ...
  pub fn tryBuyRes( self : *BuildEntry, econ : *ecn.Economy ) bool
  {
    ...
    if( self.stashedFunds < utl.EPS ){ return true; }
    ...
    inline for( 0..resTypeC )| r |
    {
      ...
      if( self.getRemainResCost( resT ) > utl.EPS ){ resTypeCount += 1.0; }
    }

    if( resTypeCount < utl.EPS ){ return true; }
  ...
  pub inline fn getBuildableWithCnst( self : *const BuildEntry ) f64
  {
    ...
    if( cnstCost < utl.EPS ){ return self.unitCount; }
  ...
  {
    return( self.unitCount < 1.0 - utl.EPS );
  }
  ...
  pub fn tryBuildUnits( self : *BuildEntry, econ : *ecn.Economy ) f64
  {
    ...
    if(   targetBuildCount < utl.EPS ){ return 0.0; }
    ...
    if( realBuildCount < 1.0 - utl.EPS ){ return 0.0; }
