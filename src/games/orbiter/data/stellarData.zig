...
pub var stellarData : utl.GenDataGrid( f64, StellarBodyName, StellarMetricEnum ) = .{};
...
pub const StellarBodyType = enum( u8 )
{
  ...
  pub inline fn getMinDisplaySize( self : StellarBodyType ) utl.Vec2
  {
    return switch( self )
    {
      .STAR      => .new( 8, 8 ),
  ...
  pub inline fn getDisplayColour( self : StellarBodyType ) utl.Colour
  {
    return switch( self )
    {
      .STAR      => .gold,
