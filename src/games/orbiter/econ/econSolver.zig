  ...
  //inline for( 0..resTypeC )| r |
    {
      ...
      inline for( 0..infTypeC )| f |
      {
        ...
        const scaling  = utl.lerp( INF_MAINT_IDLE_FACTOR, 1.0, factor );
      ...
      inline for( 0..indTypeC )| d |
      {
        ...
        const scaling  = utl.lerp( IND_MAINT_IDLE_FACTOR, 1.0, factor );
  ...
  //inline for( 0..resTypeC )| r |
    {
      ...
      if( rawTotal > utl.EPS and assemblyCap < rawTotal )
      {
...
// ================================ ACTION RATES PHASE ================================
  ...
  //utl.qlog( .DEBUG, 0, @src(), "$ LOGGING FINAL POP ACTIVITY :" );
    ...
    inline for ( 0..popTypeC )| p |
    {
      ...
      if( popC > utl.EPS ) // Skip absent population
      {
        inline for( 0..resTypeC )| r |
        {
          const resT = ResType.fromIdx( r );
  ...
  //utl.qlog( .DEBUG, 0, @src(), "$ LOGGING FINAL IND ACTIVITY :" );
    ...
    inline for( 0..indTypeC )| d |
    {
      ...
      if( indC > utl.EPS ) // Skip absent industries
      {
        inline for( 0..resTypeC )| r |
        {
          const resT = ResType.fromIdx( r );
...
//fn updateGovFinances( self : *EconSolver ) void
  ...
  fn updateResPrices( self : *EconSolver ) void
  {
    ...
    inline for( 0..resTypeC )| r |
    {
      ...
      if(      finSupply > utl.EPS ){ ratio = @min( ceil, finDemand / finSupply ); }
      else if( finDemand > utl.EPS ){ ratio = ceil; }

      const rawPrice = basePrice * @max( MIN_PRICE_FACTOR, utl.pow( f64, ratio, elasticity ));
      const oldPrice = self.econ.resState.get( .PRICE, resT );
      const newPrice = utl.lerp(  oldPrice, rawPrice, dampening ); // Lerp dampening
  ...
  fn updatePopFinances( self : *EconSolver ) void
  {
    ...
    inline for( 0..popTypeC )| p |
    {
      ...
      const isPresent : bool = ( popC > utl.EPS );
      ...
      if( popC > utl.EPS )
      {
        ...
        if(      revenue > utl.EPS ){ margin = @max( floor, profit / revenue ); }
        else if( expense > utl.EPS ){ margin =       floor; }
  ...
  fn updateIndFinances( self : *EconSolver ) void
  {
    ...
    inline for( 0..indTypeC )| d |
    {
      ...
      const isPresent : bool = ( indC > utl.EPS );
      ...
      if( indT.canBeBuiltIn( econ.location, econ.hasAtmo ))
      {
        ...
        var activityTarget = self.maxIndActivity * utl.sigmoid( margin + IND_MARGIN_OFFSET, ACT_TRGT_FACTOR );
            activityTarget = utl.clmp( activityTarget, IND_MIN_ACT_TRGT, 1.00 );
...
// Pop growth / decay factors
  const RES_MODIFIER_EXPONENT : f64 = utl.PHI; // Smooth out death rates from pop res shortages
  const JOB_MODIFIER_EXPONENT : f64 = utl.PHI; // Smooth out growth suppression from pop job shortages
  ...
  fn updatePopCount( self : *EconSolver ) void
  {
    ...
    inline for( 0..popTypeC )| p |
    {
      ...
      if( popC > utl.EPS )
      {
        ...
        const resModifier = @min( utl.pow( f64,    minResAccess, 1.0 / RES_MODIFIER_EXPONENT ), MAX_RES_MODIFIER );
        const jobModifier = @min( utl.pow( f64, 1.0 / jobAccess, 1.0 / JOB_MODIFIER_EXPONENT ), MAX_JOB_MODIFIER );
