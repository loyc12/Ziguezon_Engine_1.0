...
//allocator : std.mem.Allocator = undefined,
  ...
  {
    utl.qlog( .TRACE, @src(), "# Initializing resource manager..." );

    if( self.isInit )
    {
      utl.qlog( .WARN, @src(), "@ Resource manager is already initialized" );
      return;
    }
  ...
  //self.allocator = allocator;
    utl.qlog( .INFO, @src(), "$ Resource manager initialized !\n" );
  }

  pub fn deinit( self : *ResourceManager ) void
  {
    utl.qlog( .TRACE, @src(), "# Deinitializing resource manager..." );

    if( !self.isInit )
    {
      utl.qlog( .WARN, @src(), "@ Resource manager was not initialized" );
      return;
    }
    ...
    utl.qlog( .INFO, @src(), "$ Resource manager deinitialized !\n" );
  }
  ...
  {
    utl.log( .DEBUG, @src(), "& Adding audio from file: {s}", .{ filePath });
  ...
  {
    utl.log( .DEBUG, @src(), "#& Adding music from file: {s}", .{ filePath });
  ...
  {
    utl.log( .DEBUG, @src(), "& Adding font from file: {s}", .{ filePath });
  ...
  {
    utl.log( .DEBUG, @src(), "& Adding sprite from file: {s}", .{ filePath });
  ...
  pub fn playAudio( self : *ResourceManager, name : [ :0 ]const u8 ) void
  {
    ...
    {
      utl.log( .ERROR, @src(), "@ Audio '{s}' not found", .{ name });
      return;
    };
  ...
  pub fn playMusic( self : *ResourceManager, name : [ :0 ]const u8 ) void
  {
    ...
    {
      utl.log( .ERROR, @src(), "@ Music '{s}' not found", .{ name });
      return;
    };
  ...
  pub fn drawFromSprite( self : *ResourceManager, name : [ :0 ]const u8, index : u32, pos : utl.VecA, scale : utl.Vec2, col : utl.Colour ) void
  {
    ...
    {
      utl.log( .ERROR, @src(), "@ Sprite '{s}' not found", .{ name });
      return;
    };

    utl.log( .TRACE, @src(), "Drawing from sprite '{s}'", .{ name });
