...
- If a slice exposes a broader issue, contradiction, or dependency loop, report
  it with exact files, errors, and the smallest known blocker instead of
  expanding the architecture ad hoc.
...
- Keep compatibility semantics inside the assigned scope: avoid adding them,
  remove trivial ones touched by the slice, and report out-of-scope cleanup.
